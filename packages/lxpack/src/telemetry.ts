...
/** Assessment events added in LessonKit 1.1; forwarded until @lxpack/tracking-schema includes them. */
export const ASSESSMENT_TELEMETRY_EVENTS = ["assessment_answered"] as const;
...
function isAssessmentAnsweredData(data: unknown): data is AssessmentAnsweredData {
  return (
    typeof data === "object" &&
    data !== null &&
    typeof (data as AssessmentAnsweredData).checkId === "string" &&
    (data as AssessmentAnsweredData).checkId.length > 0
  );
}
...
export function telemetryEventToLessonkit(
  ...
  if (
    ...
    event.name === "quiz_answered" ||
    event.name === "assessment_answered"
  ) {
    const data = event.data;
    if (
      !isQuizAnsweredData(data) &&
      !isQuizCompletedData(data) &&
      !isAssessmentAnsweredData(data)
    ) {
      return null;
    }
...
/** Bridge-safe track payload for quiz/assessment answered events (@lxpack schema uses quiz_answered). */
export function answeredTelemetryToBridgeTrackEvent(
  event: TelemetryEvent,
): ReturnType<typeof mapLessonkitTelemetryToLxpack> {
