...
**Planned catalog expansion:** framework **1.2.x** (`blockCatalogVersion = 2`)
...
H5P question types implement the [question type contract](https://h5p.org/documentation/developers/contracts) (`getScore`, `getMaxScore`, `getAnswerGiven`, `resetTask`, `showSolutions`, `getXAPIData`, behaviour flags). LessonKit **1.2.x** defines an equivalent **`Assessment` runtime contract** on scored blocks:
...
| `getCurrentState()` / `resume(state)` | Resume inside compounds (1.3.x compounds) |
| `checkId` | Required on all scored blocks; sync with `lessonkit.json` |

`Quiz` / `KnowledgeCheck` (v1) are the baseline implementation; **1.2.x P0** blocks must implement the same contract.

### Framework 1.2.x — P0 checklist
...
| 7 | `Quiz` (extend) | Multiple Choice variants | Document v1 as MCQ subset; optional shuffle / multi-select in 1.2.1 |
...
**Studio 0.8.x (paired):** schema types + renderer + palette for 1.2.x P0 blocks; H5P-familiar palette labels per checklist.
...
6. Set `h5pMachineName` / `h5pAlias` on catalog JSON when 1.2.x catalog fields ship.
...
| `H5P.TrueFalse` | True/False | `TrueFalse` | B | P0 | ⬜ | 1.2.x | 0.8.x |
| `H5P.Blanks` | Fill in the Blanks | `FillInTheBlanks` | B | P0 | ⬜ | 1.2.x | 0.8.x |
| `H5P.DragQuestion` | Drag and Drop | `DragAndDrop` | B | P0 | ⬜ | 1.2.x | 0.8.x |
| `H5P.DragText` | Drag the Words | `DragTheWords` | B | P0 | ⬜ | 1.2.x | 0.8.x |
| `H5P.MarkTheWords` | Mark the Words | `MarkTheWords` | B | P0 | ⬜ | 1.2.x | 0.8.x |
| `H5P.SingleChoiceSet` | Single Choice Set | `SingleChoiceSet` | B | P1 | ⬜ | 1.2.1+ | 0.8.x |
| `H5P.Summary` | Summary | `Summary` | B | P1 | ⬜ | 1.2.1+ | 0.8.x |
| `H5P.SortParagraphs` | Sort the Paragraphs | `SortParagraphs` | B | P1 | ⬜ | 1.2.1+ | 0.8.x |
| `H5P.GuessTheAnswer` | Guess the Answer | `GuessTheAnswer` | B | P1 | ⬜ | 1.2.1+ | 0.9.x |
| `H5P.ImageMultipleHotspotQuestion` | Multimedia Choice | `MultimediaChoice` | B | P1 | ⬜ | 1.2.1+ | 0.9.x |
| `H5P.SpeakTheWords` | Speak the Words | `SpeakTheWords` | B | P2 | ⬜ | 1.3.x | 0.10.x |
| `H5P.SpeakTheWordsSet` | Speak the Words Set | `SpeakTheWordsSet` | B | P2 | ⬜ | 1.3.x | 0.10.x |
| `H5P.Dictation` | Dictation | `Dictation` | B | P2 | ⬜ | 1.3.x | 0.10.x |
| `H5P.AdvancedBlanks` / `H5P.ComplexQuestion` | Complex Fill in the Blanks | `AdvancedBlanks` | B | P2 | ⬜ | 1.4.x | 0.10.x |
| `H5P.ArithmeticQuiz` | Arithmetic Quiz | `ArithmeticQuiz` | B | P3 | ⬜ | 1.5.x | — |
| `H5P.Essay` | Essay | `Essay` | B | P3 | ⬜ | 1.5.x | 0.11.x |
| `H5P.Questionnaire` | Questionnaire | `Questionnaire` | B | P3 | ⬜ | 1.5.x | — |
| `H5P.QuestionSet` | Question Set (Quiz) | `AssessmentSequence` | A | P1 | ⬜ | 1.2.x | 0.8.x |
| — | Scenario (narrative) | `Scenario` | — | — | ✅ | 1.0 | 0.1+ |
| — | Reflection (open text) | `Reflection` | — | — | ✅ | 1.0 | — |
| `H5P.InteractiveBook` | Interactive Book | `InteractiveBook` | A | P0 | ⬜ | 1.3.x | 0.8.x |
| `H5P.CoursePresentation` | Course Presentation | `SlideDeck` | A | P0 | ⬜ | 1.4.x | 0.9.x |
| `H5P.InteractiveVideo` | Interactive Video | `InteractiveVideo` | A | P0 | ⬜ | 1.5.x | 0.10.x |
| `H5P.BranchingScenario` | Branching Scenario | `BranchingScenario` | A | P0 | ⬜ | 1.6.x | 0.11.x |
| `H5P.Column` | Column / Page | `Page` | A | P1 | 🟡 | 1.3.x | 0.8.x (`container`) |
| `H5P.GameMap` | Game Map | `GameMap` | A | P2 | ⬜ | 1.8.x | — |
...
| `H5P.ImageHotspots` | Image Hotspots | `ImageHotspots` | C | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.ImageHotspotQuestion` | Find the Hotspot | `FindHotspot` | C | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.ImageMultipleHotspotQuestion` | Find Multiple Hotspots | `FindMultipleHotspots` | C | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.ImageSlider` | Image Slider | `ImageSlider` | C | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.ImageJuxtaposition` | Image Juxtaposition | `ImageJuxtaposition` | C | P2 | ⬜ | 1.4.x | — |
| `H5P.Agamotto` | Agamotto (Image Blender) | `ImageSequence` | C | P2 | ⬜ | 1.4.x | — |
| `H5P.Collage` | Collage | `Collage` | C | P2 | ⬜ | 1.4.x | 0.9.x |
| `H5P.ImagePair` | Image Pairing | `ImagePairing` | C | P2 | ⬜ | 1.5.x | — |
| `H5P.ImageSequencing` | Image Sequencing | `ImageSequencing` | C | P2 | ⬜ | 1.5.x | — |
| `H5P.MemoryGame` | Memory Game | `MemoryGame` | C | P2 | ⬜ | 1.5.x | — |
| `H5P.IFrameEmbed` | Iframe Embedder | `Embed` | C | P3 | ⬜ | 1.6.x | — |
| `H5P.Chart` | Chart | `Chart` | C | P3 | ⬜ | 1.6.x | — |
| `H5P.Accordion` | Accordion | `Accordion` | D | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.Dialogcards` | Dialog Cards | `DialogCards` | D | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.Flashcards` | Flashcards | `Flashcards` | D | P1 | ⬜ | 1.3.x | 0.8.x |
| `H5P.Timeline` | Timeline | `Timeline` | D | P2 | ⬜ | 1.4.x | — |
| `H5P.Table` | Table | `Table` | D | P2 | ⬜ | 1.3.x | 0.8.x |
| `H5P.InformationWall` | Information Wall | `InformationWall` | D | P2 | ⬜ | 1.5.x | — |
| `H5P.AudioRecorder` | Audio Recorder | `AudioRecorder` | D | P1 | ⬜ | 1.3.x | 0.9.x |
| `H5P.ImpressivePresentation` | Slideshow (parallax) | `ParallaxSlideshow` | D | P2 | ⬜ | 1.5.x | — |
...
| `H5P.Crossword` | Crossword | `Crossword` | E | P3 | ⬜ | 1.7.x | — |
| `H5P.FindTheWords` | Find the Words | `WordSearch` | E | P3 | ⬜ | 1.7.x | — |
| `H5P.CombinationLock` | Combination Lock | `CombinationLock` | E | P3 | ⬜ | 1.7.x | — |
| `H5P.KewArCode` | KewAr Code | `QrContent` | E | P3 | ⬜ | 1.7.x | — |
| `H5P.AdventCalendar` | Advent Calendar | `AdventCalendar` | E | P3 | ⬜ | 1.8.x | — |
...
| — | Audio / Link | `audio`, `link` (Studio) | D | P1 | ⬜ | 1.3.x | 0.8.x |
...
| --- | --- | --- | --- |
| Content Type Hub | Block registry + `lessonkit blocks list` | ⬜ | Studio 0.8.x, CLI 1.7.x |
| `.h5p` package | `.lkcourse` zip + optional import adapter | ⬜ | 1.7.x |
| `semantics.json` editor | JSON Schema–driven Studio inspector | ⬜ | Studio 0.9.x |
| Question type contract | `Assessment` interface | ⬜ | 1.2.x |
| Compound allowlists | `allowedChildTypes` in catalog | ⬜ | 1.3.x |
| Resume state | `getCurrentState` / session v2 | ⬜ | 1.3.x |
...
Mirrors H5P maintainer curation ([why sub-content differs](https://snordian.de/2023/11/16/why-isnt-h5p-content-type-x-available-as-a-subcontent-option-in-content-type-y/)). Exact lists ship with each compound in **1.3.x+**.
...
## H5P import adapter (research, 1.7.x)
